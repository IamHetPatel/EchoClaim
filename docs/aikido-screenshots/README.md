...
1. Connect the repository at <https://app.aikido.dev> (GitHub OAuth, then "Connect a new
   repository").
...
4. Run AutoFix on flagged dependencies and merge the fix PRs that keep tests green.
5. Re-scan and screenshot the result as `after.png`.
6. Optionally, capture `ci-gate.png`: `aikido.yml` configured as a required CI check.
