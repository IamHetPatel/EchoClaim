# Telephony bridge: Twilio SIP to LiveKit room to agent
...
`TWILIO_ACCOUNT_SID` and `TWILIO_AUTH_TOKEN` and leave the API-Key fields empty.
...
This fetches the account and lists phone numbers:
...
`HTTP 401` means the API-Key SID/Secret pair does not match the AccountSID.

## Wiring the number to a LiveKit room

### Path A: provider-provisioned SIP (preferred)

1. Set `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET` in `.env`. If you have an
   explicit SIP endpoint (for example `sip:<id>.sip.livekit.cloud`), set `LIVEKIT_SIP_URI`
   too.
...
## Latency budget (target under 740ms total)

| Step | Budget |
| --- | --- |
| STT (Gradium / Whisper) | under 120ms |
| Gemini 2.5 Flash first token | under 220ms |
| Gradium TTS first audio (TTFT) | under 300ms |
| Network + jitter | under 100ms |

Any remaining gap is masked with filler audio (`fillers/manifest.json`). The moment a tool
call is dispatched, a short "let me just pull up the map" clip plays, which keeps perceived
latency under the roughly 500ms threshold.
...
The LiveKit side is wired. The only missing hop is Twilio to LiveKit's SIP URI. There are
three ways to demo without it.
...
Uses the machine's mic and speakers via `sounddevice`. Same agent code and bridge events
as production, without a LiveKit room or telephony.
...
3. "Connect to a custom server", then URL `wss://<your-project>.livekit.cloud` plus the API
   Key and Secret from `.env`.
4. Connect. The worker auto-dispatches and you talk to the agent in the browser.

This exercises the LiveKit project end to end and stands in for a phone caller.
...
`apply` needs a public URL serving the TwiML payload the script prints (for example
`python -m http.server 5000` plus a tunnel, a Cloudflare Worker, or a TwiML Bin).
